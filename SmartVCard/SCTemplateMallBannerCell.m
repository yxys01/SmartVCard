...
@interface SCTemplateMallBannerCell () <UICollectionViewDataSource,UICollectionViewDelegate>
...
@implementation SCTemplateMallBannerCell

+ (instancetype)cellWithTableView:(UITableView *)tableView {
    
    NSString * identifier = NSStringFromClass([SCTemplateMallBannerCell class]);
    SCTemplateMallBannerCell * cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[SCTemplateMallBannerCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
    return cell;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSInteger i = indexPath.item % 5;
    NSString *imagestr = [NSString stringWithFormat:@"Card_%ld",(long)i];
    NSLog(@"image Logo:%@ %ld",imagestr,i);
